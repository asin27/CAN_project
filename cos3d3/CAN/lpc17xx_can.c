...
static void can_SetBaudrate (LPC_CAN_TypeDef *CANx, uint32_t baudrate)
{
	 ...
	 * NT <= 24 (I guess it's a value decied on experience)
	 * TSEG1 >= 2*TSEG2
	 */
	BRFail = 1;
	for(NT=24;NT>0;NT=NT-2)
	{
		if ((result%NT)==0)
		{
			BRP = result / NT - 1;
			NT--;
			TESG2 = NT * 9 / 10 - 1;  // Sample time at 90% of the nominal time
			TSEG1 = NT - TSEG2 - 3; // NT = TSEG1+TSEG2+3 as the manual says
			BRFail = 0;
			break;
		}
	}
