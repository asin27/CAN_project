...
void winFun(){
	int i;
	for(i=0; i < 56; i++)
		LCD_DrawLine(0,i,239,i,White);
	GUI_Text(50, 25, (uint8_t *) " WE HAVE A WINNER! ", Black, White);
}

void clear(uint16_t Xpos, uint16_t Ypos, int dir){
	int i;
	for(i=60; i < 80; i++)
		LCD_DrawLine(0,i,239,i,White);
	if(dir!=-1)
		drawPlayer(Xpos, Ypos, dir, Black);
}

void initGame(){
	int i, j;
	for(i = 0, j = 265; i < 55 && j< 320; i++, j++){
		LCD_DrawLine(0, i, 239, i, Yellow);
		//LCD_DrawLine(0, j, 239, j, Yellow);
	}
	for(j = 265;j< 320; j++){
		LCD_DrawLine(0, j, 239, j, Yellow);
	}
	LCD_DrawLine(0, 55, 239, 55, Black);
  GUI_Text(50, 25, (uint8_t *) " Blind Labyrinth ", Black, Yellow);
	LCD_DrawLine(0, 264, 239, 264, Black);
	LCD_DrawLine(10, 272, 110, 272, Black);
  LCD_DrawLine(110, 272, 110, 312, Black);
  LCD_DrawLine(10, 312, 110, 312, Black);
  LCD_DrawLine(10, 272, 10, 312, Black);
  GUI_Text(25, 284, (uint8_t *) " Restart ", Black, Yellow);
  //Clear button
  LCD_DrawLine(130, 272, 230, 272, Black);
  LCD_DrawLine(230, 272, 230, 312, Black);
  LCD_DrawLine(130, 312, 230, 312, Black);
  LCD_DrawLine(130, 272, 130, 312, Black);
  GUI_Text(153, 284, (uint8_t *) " Clear ", Black, Yellow);
	GUI_Text(60,140,(uint8_t *)"Touch to start",Black,White);
  GUI_Text(76,155,(uint8_t *)"a new game",Black,White);
	
	return;
}

void restartGame(int mode){
	int i;
	if(mode){
	for(i = 0; i < 55; i++){
		LCD_DrawLine(0, i, 239, i, Yellow);
...
void drawSquare(uint16_t Xpos, uint16_t Ypos, uint16_t color){
	int x1 = Xpos*16, x2 = Xpos*16 + 15;
	int y1 = Ypos*16 +56 , y2 = Ypos*16 + 15 + 56;
	
	for(y1 = Ypos*16 +56; y1 <= y2; y1++)
		LCD_DrawLine(x1, y1, x2, y1, color);

}

void drawLineOfSquares(int level, int color){
	int j;
	for(j=3;j<12; j++)
		drawSquare(j, level, color);
}

void drawPlayer(uint16_t Xpos, uint16_t Ypos, int rot, uint16_t color){
	int x1 = Xpos*16, x2 = Xpos*16 + 15;
	int y1 = Ypos*16 +56 , y2 = Ypos*16 + 15 + 56;
	int mx = x1 + 8, my = y1 + 8;
	...
	switch(rot){
		case 0: //up
			for(y2 = Ypos*16 + 15 + 56; y2>y1 && x2>=mx; x1++, x2--, y2--){
				LCD_DrawLine(x1, y2, x2, y2, color);
				LCD_DrawLine(x2, y2, mx, y1, color);
				LCD_DrawLine(x1, y2, mx, y1, color);
			}
			break;
		case 1: //right
			for(x1 = Xpos*16; x1 < x2 && y2>=my; x1++, y1++, y2--){
				LCD_DrawLine(x1, y1, x1, y2, color); 
				LCD_DrawLine(x1, y1, x2, my, color); 
				LCD_DrawLine(x1, y2, x2, my, color);
			}
			break;
		case 2: //down
			for(y1 = Ypos*16 + 56; y1<y2 && x2>=mx; x1++, x2--, y1++){
				LCD_DrawLine(x1, y1, x2, y1, color); 
				LCD_DrawLine(x1, y1, mx, y2, color); 
				LCD_DrawLine(x2, y1, mx, y2, color);
			}
			break;
		case 3: //left
			for(x2 = Xpos*16 + 15; x2>x1 && y2>=my; y1++, x2--, y2--){
				LCD_DrawLine(x2, y1, x2, y2, color); 
				LCD_DrawLine(x2, y2, x1, my, color); 
				LCD_DrawLine(x2, y1, x1, my, color);
			}
			break;
		default:
			break;
