...
// pass to the functions two opposite points to draw a rectangle

void fillRectangle(uint16_t Xini, uint16_t Yini, uint16_t Xfin, uint16_t Yfin, uint16_t color, uint16_t background) {
	int i, temp;
	...
	for(i = Yini; i < Yfin; i++){
		LCD_DrawLine(Xini, i, Xfin, i, color);
	}
	for(i = 0; i < Yini; i++){
		LCD_DrawLine(Xini, i, Xfin, i, background);
	}
}
